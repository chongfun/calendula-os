//! Xteink X3 board values.
//!
//! The X3 shares the X4's ESP32-C3 pin map; only the ADC button-ladder
//! windows, the recovery combo windows, and the dram2 radio-heap budget
//! differ (the X3 framebuffer is larger).
...
/// Everything `main` needs off the chip, with the X3's pin map already
...
/// Claims the chip's peripherals and wires them to the X3's pin map. The
...
pub(crate) const NAV_BACK_MIN_MV: u16 = 2406;
pub(crate) const NAV_BACK_MAX_MV: u16 = 2706;

pub(crate) const NAV: &[Band] = &[
    // X3 front-button ladder on GPIO1, scaled from the X3's resistor ladder
    // to the 12-bit esp-hal ADC reads.
    Band {
        min: NAV_BACK_MIN_MV,
        max: NAV_BACK_MAX_MV,
...
pub(crate) const PAGE: &[Band] = &[
    // X3 side-button ladder on GPIO2.
    Band {
        min: 1509,
        max: 1809,
...
pub(crate) const RECOVERY_NAV_MV: RangeInclusive<u16> = 2406..=2706;
pub(crate) const RECOVERY_PAGE_MV: RangeInclusive<u16> = 1509..=1809;

/// Bytes claimed from `dram2_seg` for the radio heap. The X3 framebuffer is
/// 4,272 bytes larger than the X4's, so the heap budget shrinks to keep the
/// framebuffer + radio heap under the ~64.8 KB dram2 segment boundary (see
/// `sync_mem`).
pub(crate) const DRAM2_HEAP_BYTES: usize = 12 * 1024;
