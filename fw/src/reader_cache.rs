...
use embedded_sdmmc::{File, Mode, SdCard, TimeSource, VolumeIdx, VolumeManager};
...
const INITIAL_READER_CACHE_PAGES: usize = 5;
...
pub(crate) fn load_book_preview(
    epd: &mut Epd,
    ...
    esp_println::println!("epub: open index {} chapter {}", index, requested_chapter);
        ...
        let load_result = if in_books_dir {
            match root.open_dir("BOOKS") {
                Ok(books) => match books.open_file_in_dir(open_name.as_str(), Mode::ReadOnly) {
                    Ok(file) => load_epub_preview_from_file(
                        file,
                        &display_name,
                        requested_chapter,
                        library,
...
fn load_epub_preview_from_file<
    D,
    T,
    ...
    T: TimeSource,
{
        ...
        .filter(|(_, item)| !item.href.is_empty() && !spine_item_is_navigation(item, &package))
    {
        ...
        if library.page_count >= INITIAL_READER_CACHE_PAGES {
            break;
        }
    }
    ...
    if library.block_count > 0 {
        ...
        esp_println::println!(
            "epub: initial cache ready after {} ms ({} page(s), {} block(s))",
            open_started.elapsed().as_millis(),
            library.page_count,
            library.block_count
        );
