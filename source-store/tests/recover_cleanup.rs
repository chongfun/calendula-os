...
fn cleanup_reclaims_superseded_generation_and_frees_its_slot() {
    ...
    let report = run_cleanup(&root, &idem, &mut ws).expect("cleanup");
    ...
    // next create.
    let report = run_cleanup(&root, &idem, &mut ws).expect("cleanup again");
...
fn power_cut_during_cleanup_preserves_authority_and_converges() {
    ...
    disk.start_logging();
    {
        ...
        let root = open_root(&mgr);
        let mut ws = workspace();
        let idem = IdempotencyStore::load(&root, &mut ws).expect("idem");
        run_cleanup(&root, &idem, &mut ws).expect("cleanup");
    }
    ...
    for cut in (0..=log.len()).step_by(2) {
        ...
        let mut ws = workspace();
        let idem = IdempotencyStore::load(&root, &mut ws).expect("idem");
        run_cleanup(&root, &idem, &mut ws).expect("cleanup rerun");
        let rerun = run_cleanup(&root, &idem, &mut ws).expect("cleanup idempotent");
