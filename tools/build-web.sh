...
# static site (index.html + one wasm per board) into OUT_DIR.
#
...
OUT_DIR="${1:-_site}"
TOOLCHAIN="${TOOLCHAIN:-nightly-2025-10-01}"
...
mkdir -p "$OUT_DIR"
cp "$ROOT/web/index.html" "$OUT_DIR/"
...
build x4
build x3 --no-default-features --features board-x3

echo "Wrote $OUT_DIR/{index.html, x4_web_emulator.wasm, x3_web_emulator.wasm}"
