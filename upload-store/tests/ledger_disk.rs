...
//! not at all, that a power cut anywhere in a rewrite leaves the generation
//! that stood, that a damaged newer side hands over to the older one, and
//! that the join between a freshly written catalog and the ledger gives each
//! row the id it had and mints for the rest.
//!
...
use proto::catalog::{
    catalog_record_book_id, encode_catalog_placeholder_header, encode_catalog_record,
    CATALOG_HEADER_BYTES, CATALOG_RECORD_BYTES,
};
use proto::identity::{BookId, LedgerRecord, LEDGER_HEADER_BYTES};
...
fn read_exact(
    file: &embedded_sdmmc::File<'_, SharedDisk, StaticTime, 8, 8, 1>,
    mut out: &mut [u8],
) {
    while !out.is_empty() {
        let read = file.read(out).expect("read");
        assert!(read > 0, "file ended early");
        let rest = out;
        out = &mut rest[read..];
    }
}

/// Write `rows` as a fresh, uncommitted `CATALOG.BIN` the way the scan
/// does, call `arm` (which is where a test cuts the power), then run the
/// identity join over it with a scratch of `scratch_len` bytes. Hands back
/// what the join reported and the id every row ended up carrying.
fn scan(
    root: &Dir<'_>,
    rows: &[Row],
    scratch_len: usize,
    ...
    let assigned = ledger::assign_book_ids(root, &file, rows.len() as u16, &mut scratch, random)?;
...
/// Every live record, in ledger order.
fn records(root: &Dir<'_>) -> Vec<(BookRoot, String, u32, BookId)> {
    let mut out = Vec::new();
...
/// so the reconciliation that recognises a move has something to match.
#[test]
fn a_moved_or_resized_copy_is_minted_afresh_and_the_old_record_stays() {
    ...
    assert!(
        live.iter().any(
            |(_, locator, _, id)| locator == "History/Rome/SPQR.epub" && Some(*id) == before[3]
        ),
        "the departed copy's record stands"
    );
}
...
/// The scratch bounds the slice, not the library: two keys' worth of
/// scratch joins five rows in three passes over the ledger. No scratch at
/// all is refused rather than looped over.
#[test]
fn rows_beyond_one_scratch_slice_are_still_joined() {
    ...
    let tiny = 2 * proto::identity::ROW_KEY_BYTES;
    ...
    assert_eq!(
        scan(&root, &SHELF, 0, &mut random, || {}).err(),
        Some(LedgerFault::Scratch)
    );
}

/// A power cut at every write of a rewrite. Whatever the cut left, the
/// generation that stood before it still reads back with every id, or the
/// new one landed whole; and once the power is back a scan finishes the job.
#[test]
fn a_power_cut_anywhere_in_a_rewrite_keeps_the_generation_that_stood() {
    let disk = new_card();
    let mut random = entropy();
    ...
    // How many writes the rewrite takes when nothing cuts it.
    let uncut = {
        let mgr = open_mgr(&disk);
        let root = open_root(&mgr);
    ...
    for cut in 1..=uncut {
        ...
        // Power is back. Whatever landed must be one of the two legal
        // states, and never a shorter or mixed library.
        let mgr = open_mgr(&disk);
        let root = open_root(&mgr);
        match outcome {
            Ok((assigned, _)) => {
            ...
            Err(fault) => {
                ...
                assert_eq!(
                    records(&root),
                    standing,
                    "cut at write {cut}: the standing generation is intact"
                );
            }
        }
        // The next scan finishes what the cut interrupted, over whichever
        // side the cut damaged.
        ...
        for (index, (_, _, _, id)) in standing.iter().enumerate() {
            assert_eq!(
                ids[index],
                Some(*id),
...
/// A generation whose header is fine and whose records are not is not a
/// shorter library; it is no generation, and the older side answers. The
/// next rewrite goes over the damaged side, so the ledger heals itself.
#[test]
fn a_damaged_newer_generation_hands_over_to_the_older_one() {
    let disk = new_card();
    let mgr = open_mgr(&disk);
    let root = open_root(&mgr);
    let mut random = entropy();
    scan(&root, &SHELF[..3], ARENA, &mut random, || {}).unwrap();
    let older = records(&root);
...

    {
        ...
        let file = cache_root
            .open_file_in_dir(LEDGER_FILES[1], Mode::ReadWriteAppend)
            .unwrap();
        // One byte inside the fourth record's locator.
        file.seek_from_start(
            (LEDGER_HEADER_BYTES + 3 * proto::identity::LEDGER_RECORD_BYTES + 25) as u32,
        )
        .unwrap();
        file.write(&[0xFF]).unwrap();
    }
    assert_eq!(generation(&root), Some((1, 3, LEDGER_FILES[0])));
    assert_eq!(records(&root), older);

    let (assigned, healed) = scan(&root, &SHELF, ARENA, &mut random, || {}).unwrap();
    assert_eq!(assigned.matched, 3);
    assert_eq!(assigned.minted, 2);
    assert_eq!(generation(&root), Some((2, 5, LEDGER_FILES[1])));
    assert_eq!(&healed[..3], &ids[..3], "the older generation's ids held");
    assert!(
        healed[3] != ids[3] && healed[4] != ids[4],
        "the ids the damaged generation held are gone with it"
    );
}
...
fn a_place_named_twice_takes_the_first_record_and_reports_it() {
    ...
    let row = SHELF[3];
    ledger::write_generation(&root, None, |writer| {
