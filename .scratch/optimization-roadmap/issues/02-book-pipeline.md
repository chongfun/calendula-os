...
**Measured on X3 2026-07-25 (main 95f4bf2, 11.7 MB baseline book):** Type Size changes replayed in **24.7 s** (736 pages / 82 sections) and **27.1 s** (1240 pages / 100 sections) — ~280–300 ms per section. The replay path is proven genuine by read volume: 2.8–3.8 MB read per rebuild vs the 11.7 MB source zip a fallback would stream. Conclusion: capture/replay works, but nearly all build cost is downstream of the capture point (wrap + section writes), which the zip/inflate/XML skip cannot touch — **this is the "replay still slow" outcome, so B7 is promoted (below)**. Not yet measured: the full-build-vs-replay ratio at identical settings (caches were warm all session). Original design kept for reference:
